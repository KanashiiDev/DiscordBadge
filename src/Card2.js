...
class Card {
	constructor({
		...
		if(customBg && fgColor && fgColor !== "#fff0") {
			...
			if(this.height == 150) {
				this.fgbg = "height:80px;background:" + changeColorAlpha(this.fgColor, this.fgopacity) + ";margin: 10px;border-radius: 4px;";
			}
		}
		else if(this.height == 150) {
			this.fgbg = "height:80px;background:" + this.fgColor + ";margin: 10px;border-radius: 4px;";
		}
	}
	render() {
	return `
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" width="300" height="${this.height}" viewBox="0 0 300 ${this.height}" role="img">
  <style>
  svg#Layer_1{
    min-width:250px;
  ...
  #bgcolor{
    ${this.bgmode ? 'height:100%;background:' + changeColorAlpha(this.bgColor, this.fgopacity) + ';' : ""}
  }
  .namedetails {
    ${this.nameheight}
  }
  ...
  .test2{
    ...
    ${this.bgmode == 2 ? 'padding: 0px 10px 0px 10px;' : ""}
  }
  .detailimgDiv {
  ...
  <img id="detail-image" height="28px" width="28px" src="${this.detailsImage}"/>
  </div>
